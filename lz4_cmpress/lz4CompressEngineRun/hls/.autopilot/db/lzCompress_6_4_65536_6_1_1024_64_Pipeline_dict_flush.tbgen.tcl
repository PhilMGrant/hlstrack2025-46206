set moduleName lzCompress_6_4_65536_6_1_1024_64_Pipeline_dict_flush
set isTopModule 0
set isCombinational 0
set isDatapathOnly 0
set isPipelined 1
set pipeline_type loop_auto_rewind
set FunctionProtocol ap_ctrl_hs
set isOneStateSeq 0
set ProfileFlag 0
set StallSigGenFlag 0
set isEnableWaveformDebug 1
set hasInterrupt 0
set DLRegFirstOffset 0
set DLRegItemOffset 0
set svuvm_can_support 1
set cdfgNum 19
set C_modelName {lzCompress<6, 4, 65536, 6, 1, 1024, 64>_Pipeline_dict_flush}
set C_modelType { void 0 }
set ap_memory_interface_dict [dict create]
dict set ap_memory_interface_dict dict_3 { MEM_WIDTH 408 MEM_SIZE 13056 MASTER_TYPE BRAM_CTRL MEM_ADDRESS_MODE WORD_ADDRESS PACKAGE_IO port READ_LATENCY 0 }
dict set ap_memory_interface_dict dict_2 { MEM_WIDTH 408 MEM_SIZE 13056 MASTER_TYPE BRAM_CTRL MEM_ADDRESS_MODE WORD_ADDRESS PACKAGE_IO port READ_LATENCY 0 }
dict set ap_memory_interface_dict dict_1 { MEM_WIDTH 408 MEM_SIZE 13056 MASTER_TYPE BRAM_CTRL MEM_ADDRESS_MODE WORD_ADDRESS PACKAGE_IO port READ_LATENCY 0 }
dict set ap_memory_interface_dict dict { MEM_WIDTH 408 MEM_SIZE 13056 MASTER_TYPE BRAM_CTRL MEM_ADDRESS_MODE WORD_ADDRESS PACKAGE_IO port READ_LATENCY 0 }
set C_modelArgList {
	{ dict_3 int 408 regular {array 256 { 3 0 } 0 1 }  }
	{ dict_2 int 408 regular {array 256 { 3 0 } 0 1 }  }
	{ dict_1 int 408 regular {array 256 { 3 0 } 0 1 }  }
	{ dict int 408 regular {array 256 { 3 0 } 0 1 }  }
}
set hasAXIMCache 0
set l_AXIML2Cache [list]
set AXIMCacheInstDict [dict create]
set C_modelArgMapList {[ 
	{ "Name" : "dict_3", "interface" : "memory", "bitwidth" : 408, "direction" : "WRITEONLY"} , 
 	{ "Name" : "dict_2", "interface" : "memory", "bitwidth" : 408, "direction" : "WRITEONLY"} , 
 	{ "Name" : "dict_1", "interface" : "memory", "bitwidth" : 408, "direction" : "WRITEONLY"} , 
 	{ "Name" : "dict", "interface" : "memory", "bitwidth" : 408, "direction" : "WRITEONLY"} ]}
# RTL Port declarations: 
set portNum 22
set portList { 
	{ ap_clk sc_in sc_logic 1 clock -1 } 
	{ ap_rst sc_in sc_logic 1 reset -1 active_high_sync } 
	{ ap_start sc_in sc_logic 1 start -1 } 
	{ ap_done sc_out sc_logic 1 predone -1 } 
	{ ap_idle sc_out sc_logic 1 done -1 } 
	{ ap_ready sc_out sc_logic 1 ready -1 } 
	{ dict_3_address1 sc_out sc_lv 8 signal 0 } 
	{ dict_3_ce1 sc_out sc_logic 1 signal 0 } 
	{ dict_3_we1 sc_out sc_logic 1 signal 0 } 
	{ dict_3_d1 sc_out sc_lv 408 signal 0 } 
	{ dict_2_address1 sc_out sc_lv 8 signal 1 } 
	{ dict_2_ce1 sc_out sc_logic 1 signal 1 } 
	{ dict_2_we1 sc_out sc_logic 1 signal 1 } 
	{ dict_2_d1 sc_out sc_lv 408 signal 1 } 
	{ dict_1_address1 sc_out sc_lv 8 signal 2 } 
	{ dict_1_ce1 sc_out sc_logic 1 signal 2 } 
	{ dict_1_we1 sc_out sc_logic 1 signal 2 } 
	{ dict_1_d1 sc_out sc_lv 408 signal 2 } 
	{ dict_address1 sc_out sc_lv 8 signal 3 } 
	{ dict_ce1 sc_out sc_logic 1 signal 3 } 
	{ dict_we1 sc_out sc_logic 1 signal 3 } 
	{ dict_d1 sc_out sc_lv 408 signal 3 } 
}
set NewPortList {[ 
	{ "name": "ap_clk", "direction": "in", "datatype": "sc_logic", "bitwidth":1, "type": "clock", "bundle":{"name": "ap_clk", "role": "default" }} , 
 	{ "name": "ap_rst", "direction": "in", "datatype": "sc_logic", "bitwidth":1, "type": "reset", "bundle":{"name": "ap_rst", "role": "default" }} , 
 	{ "name": "ap_start", "direction": "in", "datatype": "sc_logic", "bitwidth":1, "type": "start", "bundle":{"name": "ap_start", "role": "default" }} , 
 	{ "name": "ap_done", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "predone", "bundle":{"name": "ap_done", "role": "default" }} , 
 	{ "name": "ap_idle", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "done", "bundle":{"name": "ap_idle", "role": "default" }} , 
 	{ "name": "ap_ready", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "ready", "bundle":{"name": "ap_ready", "role": "default" }} , 
 	{ "name": "dict_3_address1", "direction": "out", "datatype": "sc_lv", "bitwidth":8, "type": "signal", "bundle":{"name": "dict_3", "role": "address1" }} , 
 	{ "name": "dict_3_ce1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "dict_3", "role": "ce1" }} , 
 	{ "name": "dict_3_we1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "dict_3", "role": "we1" }} , 
 	{ "name": "dict_3_d1", "direction": "out", "datatype": "sc_lv", "bitwidth":408, "type": "signal", "bundle":{"name": "dict_3", "role": "d1" }} , 
 	{ "name": "dict_2_address1", "direction": "out", "datatype": "sc_lv", "bitwidth":8, "type": "signal", "bundle":{"name": "dict_2", "role": "address1" }} , 
 	{ "name": "dict_2_ce1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "dict_2", "role": "ce1" }} , 
 	{ "name": "dict_2_we1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "dict_2", "role": "we1" }} , 
 	{ "name": "dict_2_d1", "direction": "out", "datatype": "sc_lv", "bitwidth":408, "type": "signal", "bundle":{"name": "dict_2", "role": "d1" }} , 
 	{ "name": "dict_1_address1", "direction": "out", "datatype": "sc_lv", "bitwidth":8, "type": "signal", "bundle":{"name": "dict_1", "role": "address1" }} , 
 	{ "name": "dict_1_ce1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "dict_1", "role": "ce1" }} , 
 	{ "name": "dict_1_we1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "dict_1", "role": "we1" }} , 
 	{ "name": "dict_1_d1", "direction": "out", "datatype": "sc_lv", "bitwidth":408, "type": "signal", "bundle":{"name": "dict_1", "role": "d1" }} , 
 	{ "name": "dict_address1", "direction": "out", "datatype": "sc_lv", "bitwidth":8, "type": "signal", "bundle":{"name": "dict", "role": "address1" }} , 
 	{ "name": "dict_ce1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "dict", "role": "ce1" }} , 
 	{ "name": "dict_we1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "dict", "role": "we1" }} , 
 	{ "name": "dict_d1", "direction": "out", "datatype": "sc_lv", "bitwidth":408, "type": "signal", "bundle":{"name": "dict", "role": "d1" }}  ]}

set RtlHierarchyInfo {[
	{"ID" : "0", "Level" : "0", "Path" : "`AUTOTB_DUT_INST", "Parent" : "", "Child" : ["1"],
		"CDFG" : "lzCompress_6_4_65536_6_1_1024_64_Pipeline_dict_flush",
		"Protocol" : "ap_ctrl_hs",
		"ControlExist" : "1", "ap_start" : "1", "ap_ready" : "1", "ap_done" : "1", "ap_continue" : "0", "ap_idle" : "1", "real_start" : "0",
		"Pipeline" : "None", "UnalignedPipeline" : "0", "RewindPipeline" : "0", "ProcessNetwork" : "0",
		"II" : "0",
		"VariableLatency" : "1", "ExactLatency" : "-1", "EstimateLatencyMin" : "258", "EstimateLatencyMax" : "258",
		"Combinational" : "0",
		"Datapath" : "0",
		"ClockEnable" : "0",
		"HasSubDataflow" : "0",
		"InDataflowNetwork" : "0",
		"HasNonBlockingOperation" : "0",
		"IsBlackBox" : "0",
		"Port" : [
			{"Name" : "dict_3", "Type" : "Memory", "Direction" : "O"},
			{"Name" : "dict_2", "Type" : "Memory", "Direction" : "O"},
			{"Name" : "dict_1", "Type" : "Memory", "Direction" : "O"},
			{"Name" : "dict", "Type" : "Memory", "Direction" : "O"}],
		"Loop" : [
			{"Name" : "dict_flush", "PipelineType" : "UPC",
				"LoopDec" : {"FSMBitwidth" : "1", "FirstState" : "ap_ST_fsm_state1", "FirstStateIter" : "", "FirstStateBlock" : "ap_ST_fsm_state1_blk", "LastState" : "ap_ST_fsm_state1", "LastStateIter" : "", "LastStateBlock" : "ap_ST_fsm_state1_blk", "QuitState" : "ap_ST_fsm_state1", "QuitStateIter" : "", "QuitStateBlock" : "ap_ST_fsm_state1_blk", "OneDepthLoop" : "1", "has_ap_ctrl" : "1", "has_continue" : "0"}}]},
	{"ID" : "1", "Level" : "1", "Path" : "`AUTOTB_DUT_INST.flow_control_loop_pipe_sequential_init_U", "Parent" : "0"}]}


set ArgLastReadFirstWriteLatency {
	lzCompress_6_4_65536_6_1_1024_64_Pipeline_dict_flush {
		dict_3 {Type O LastRead -1 FirstWrite 0}
		dict_2 {Type O LastRead -1 FirstWrite 0}
		dict_1 {Type O LastRead -1 FirstWrite 0}
		dict {Type O LastRead -1 FirstWrite 0}}}

set hasDtUnsupportedChannel 0

set PerformanceInfo {[
	{"Name" : "Latency", "Min" : "258", "Max" : "258"}
	, {"Name" : "Interval", "Min" : "258", "Max" : "258"}
]}

set PipelineEnableSignalInfo {[
]}

set Spec2ImplPortList { 
	dict_3 { ap_memory {  { dict_3_address1 MemPortADDR2 1 8 }  { dict_3_ce1 MemPortCE2 1 1 }  { dict_3_we1 MemPortWE2 1 1 }  { dict_3_d1 MemPortDIN2 1 408 } } }
	dict_2 { ap_memory {  { dict_2_address1 MemPortADDR2 1 8 }  { dict_2_ce1 MemPortCE2 1 1 }  { dict_2_we1 MemPortWE2 1 1 }  { dict_2_d1 MemPortDIN2 1 408 } } }
	dict_1 { ap_memory {  { dict_1_address1 MemPortADDR2 1 8 }  { dict_1_ce1 MemPortCE2 1 1 }  { dict_1_we1 MemPortWE2 1 1 }  { dict_1_d1 MemPortDIN2 1 408 } } }
	dict { ap_memory {  { dict_address1 MemPortADDR2 1 8 }  { dict_ce1 MemPortCE2 1 1 }  { dict_we1 MemPortWE2 1 1 }  { dict_d1 MemPortDIN2 1 408 } } }
}
